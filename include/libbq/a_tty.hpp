...
namespace BQ {
class a_tty;
/** Defines a "line discipline" handler. **/
 ...
 **/
class a_tty {
  /** Internal array of functions that handle control characters. **/
protected:
  ASCII_ControlChar_handler_t line_discipline[0x20]{NULL};
  /**
...
public:
   ...
   */
public:
  virtual signed int cursor_move(signed int);
  /**
   * @brief query the position of the tty cursor.
   *
   */
public:
  virtual int32_xy cursor_position();
  /**
   ...
   */
public:
  virtual int32_xy cursor_position(int32_xy);
  /**
   * @brief Defines if printing order is "ABCDEF"...-> (true) or <-..."FEDCBA"
   * (false)
   */
public:
  virtual bool left_to_right();
  /**
   ...
   */
public:
  virtual bool left_to_right(bool);
  /**
   * @brief The inverse of `left_to_right()`
   * @note Controlled via `left_to_right(bool)`
   */
public:
  virtual bool right_to_left();
  /** @brief Does the text scroll upwards towards Y=0 or away from Y=0?**/
public:
  virtual bool line_feeds_up();
  /** @brief Does the text scroll upwards towards Y=0 or away from Y=0?**/
public:
  virtual bool line_feeds_down();
  /** How wide are tabulations on this TTY? */
public:
  virtual int h_tabulation_size();
  /** How wide are vertical tabulations on this TTY? */
public:
  virtual int v_tabulation_size();
  /**
   * @brief Place a character at a certain linear `size_t` index in the tty
   * buffer.
   * @note This does not interpret the character; it simply places whatever
   * value specified by the `char` into the tty buffer.
   */
public:
  virtual int put_at(int, char);
  /**
   * @brief Place a character at a certain `int32_xy` position within the tty
   * buffer.
   * @note the `position` may be clipped if outside of the `screen_size()` of
   * the buffer.
   */
public:
  virtual int put_at(int32_xy, char);
  /**
   ...
   * @note this will return -1 if the int32_xy value is greater than
   * `screen_size()` or has either negative X or Y.
   * @return {int} index of xy position in tty linear buffer. `-1` if clipped.
   */
public:
  virtual int index_at(int32_xy);
  /**
   ...
   */
public:
  virtual bool valid_index(int index);
  /**
...
public:
   ...
   *          and the extending class can safely ignore and return printing
   * nothing.
...
public:
   ...
   */
public:
  virtual int put(const char *buffer, size_t length);
  /**
   ...
   */
public:
  virtual int put(const char *buffer);
};
} // namespace BQ
#endif
